...
\begin{tabular}{llc}
\toprule
\textbf{File} & \textbf{Purpose} & \textbf{Theorems \& Lemmas} \\
\midrule
\texttt{Protocol.lean} & Core 6-packet structure & 4 \\
\texttt{Dependencies.lean} & Bilateral T creation & 14 \\
\texttt{ProofStapling.lean} & T\_B proves D\_A delivered & 5 \\
\texttt{Channel.lean} & Fair-lossy model & 29 \\
\texttt{Emergence.lean} & Tripartite attack key & 26 \\
\texttt{Bilateral.lean} & Symmetric outcomes & 9 \\
\texttt{Exhaustive.lean} & All 64 states symmetric & 29 \\
\texttt{Theseus.lean} & Protocol of Theseus & 10 \\
\texttt{Gray.lean} & Closure failure (Theorem~\ref{thm:closure-failure}) & 27 \\
\texttt{GrayCore.lean} & Local views & 16 \\
\texttt{GrayInterp.lean} & True local views & 65 \\
\texttt{LocalDetect.lean} & Attack key detectability & 41 \\
\texttt{Epistemic.lean} & Knowledge levels & 28 \\
\texttt{Solution.lean} & Main theorem synthesis & 17 \\
\texttt{TGPMinimal.lean} & 8-bit crash safety & 9 \\
\texttt{FairExchange.lean} & Bilateral fair-exchange extension & 11 \\
\texttt{FairExchangeStandalone.lean} & Standalone fair-exchange variant & 14 \\
\midrule
...
pub struct BilateralSyncReceipt {
    tgp_proof: TripleProof,      // Coordination guarantee
...
TGP-Native (TripleProof only) & $\sim$2.5 KB & $\sim$250 KB \\
\midrule
...
\begin{itemize}
    \item One UDP socket
    \item TripleProof HashMap (authorized peers)
...
\paragraph{Reproducing the Timed-Regime Experiments (\S\ref{sec:timed-regime}).}
The $\varepsilon(\tau)$ curve (Figure~\ref{fig:eps-tau}) and the adversary comparison (Table~\ref{tab:adaptive}) are generated by the experiment harnesses in \texttt{python/}, which drive the repository's own \texttt{SimulatedParty}:
\begin{verbatim}
# Single-condition verification (Conditions 1-3: oblivious vs adaptive)
python dawn_experiment.py

# eps(tau) sweep across loss rates (generates Figure fig:eps-tau data)
python dawn_sweep.py
\end{verbatim}
Runs are seeded and deterministic. The figure and Table~\ref{tab:adaptive} report \texttt{dawn\_sweep.py} output, which seeds each cell with \texttt{seed = int(p*1000) + dawn} (e.g.\ $p{=}0.5,\ \tau{=}8 \Rightarrow 47.9\%$). The standalone \texttt{dawn\_experiment.py} seeds with \texttt{seed = dawn} and reports the same knife-edge at $49.3\%$; the ${\sim}2$pp gap is Monte-Carlo variance at the peak, not a discrepancy.
