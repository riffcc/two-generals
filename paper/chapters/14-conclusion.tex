...
\begin{enumerate}
    ...
    \item \textbf{Achieving an epistemic fixpoint:} The bilateral triple pair $(T_A, T_B)$ is a finite cryptographic artifact encoding the infinite epistemic hierarchy that Gray claimed could not be achieved (Theorem~\ref{thm:fixpoint}).
\end{enumerate}

The result: in the terminal-state regime, safety is \emph{deterministic}---every completed execution yields a symmetric outcome, including under total channel failure (Theorem~\ref{thm:safety}). Under a bounded deadline, safety holds with probability $1 - 10^{-1565}$ (Theorem~\ref{thm:liveness}), with \textbf{zero asymmetric outcomes} observed across all testing.
...
The Lean 4 formalization (\texttt{lean4/v2/}) provides machine-verified guarantees with \textbf{zero \texttt{sorry} statements}---the proofs are complete with no deferred obligations. The verification includes \textbf{350+ theorems and lemmas across 17 files}, covering:
...
Fair Exchange & \texttt{FairExchange.lean} & Bilateral fair-exchange extension \\
 & \texttt{FairExchangeStandalone.lean} & Standalone fair-exchange variant \\
\bottomrule
\end{tabular}
\end{center}
