...
\midrule
Formal correctness proof & 12 theorems + 2 axioms (\texttt{TGPMinimal.lean}), 0 \texttt{sorry} \\
...
\paragraph{Proven Theorems in \texttt{TGPMinimal.lean} (12 total, 0 \texttt{sorry}).}
...
\midrule
\texttt{advance\_from\_commit\_needs\_seen} & To leave COMMIT, must have observed $\geq$ COMMIT \\
\texttt{advance\_from\_double\_needs\_seen} & To leave DOUBLE, must have observed $\geq$ DOUBLE \\
...
\texttt{action\_requires\_both\_alive} & Coordinated action requires both parties active \\
...
All phase-advancement and crash-safety properties are proven, not assumed: \texttt{advance\_from\_*\_needs\_seen} from the definition of \texttt{can\_advance}, and \texttt{action\_requires\_both\_alive} and \texttt{crash\_prevents\_dangerous\_action} (Theorem~\ref{thm:crash-safety}) from \texttt{can\_execute\_action}. \texttt{TGPMinimal.lean} contains twelve theorems in total (0 \texttt{sorry}), additionally including \texttt{bilateral\_attack\_guarantee} and \texttt{protocol\_correct}. The trusted base is two irreducible modeling axioms only---the channel-authenticity pair \texttt{saw\_phase\_means\_they\_sent}---which is precisely the pre-authentication guarantee a DAL-A channel provides; fair-lossy delivery is a property of the channel itself (formalized in \texttt{Channel.lean}), not an axiom of the 8-bit state model.
