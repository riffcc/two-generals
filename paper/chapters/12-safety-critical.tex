...
\midrule
Formal correctness proof & 9 theorems (\texttt{TGPMinimal.lean}), 0 \texttt{sorry} \\
...
\paragraph{Proven Theorems in \texttt{TGPMinimal.lean} (9 total, 0 \texttt{sorry}).}
...
Of the entries above, \texttt{double\_needs\_their\_commit} and \texttt{ready\_needs\_their\_double} are channel axioms---the phase-advancement premises (physical properties of the pre-authenticated channel), not derived theorems; the remaining rows are proven. \texttt{TGPMinimal.lean} contains nine theorems in total (0 \texttt{sorry}), including additionally \texttt{crash\_prevents\_dangerous\_action} (Theorem~\ref{thm:crash-safety}), \texttt{bilateral\_attack\_guarantee}, and \texttt{protocol\_correct}.
