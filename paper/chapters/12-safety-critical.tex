...
\midrule
Formal correctness proof & 12 theorems + 3 axioms (\texttt{TGPMinimal.lean}), 0 \texttt{sorry} \\
...
All phase-advancement and crash-safety properties are proven, not assumed: \texttt{advance\_from\_*\_needs\_seen} from the definition of \texttt{can\_advance}, and \texttt{action\_requires\_both\_alive} and \texttt{crash\_prevents\_dangerous\_action} (Theorem~\ref{thm:crash-safety}) from \texttt{can\_execute\_action}. \texttt{TGPMinimal.lean} contains twelve theorems in total (0 \texttt{sorry}), additionally including \texttt{bilateral\_attack\_guarantee} and \texttt{protocol\_correct}. The trusted base is three irreducible modeling axioms only---two channel-authenticity axioms (\texttt{saw\_phase\_means\_they\_sent}) and the fair-lossy flooding guarantee---which are precisely the physical assumptions a pre-authenticated DAL-A channel provides.
